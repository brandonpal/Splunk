...
`appserver/static/visualizations/stat_box/visualization.js`, so no build
...
`appserver/static/visualizations/stat_box/visualization.js`. Commit the
...
1. Package the app:
   ```
   tar -czf stat_box_viz.tar.gz stat_box_viz
   ```
...
`stat_box_viz.stat_box`) and bind a search that returns **one row** with the
value to display. All configuration is available in the visualization's
configuration panel ("Stat Box: Title", "Stat Box: Number", "Stat Box:
...
Set **Value field** in the "Stat Box: Number" section to target a specific
